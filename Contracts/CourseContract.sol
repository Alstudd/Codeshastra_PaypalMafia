...
contract CourseMarketplace {
    ...
    event CourseCreated(address indexed instructor, uint256 courseId, string title, uint256 price);
    event CoursePurchased(address indexed student, uint256 courseId, address indexed instructor, uint256 amountPaid);
    ...
    function createCourse(string memory _title, uint256 _price) public {
        uint256 courseId = uint256(keccak256(abi.encodePacked(msg.sender, _title, block.timestamp)));
    ...
    function buyCourse(uint256 _courseId) public payable {
        ...
        emit CoursePurchased(msg.sender, _courseId, courses[_courseId].instructor, msg.value);
    }
